...
function oq.channel_join( chan_name, pword )
  local n = strlower( chan_name ) ;
  if ((n == OQ_REALM_CHANNEL) and (oq._inside_instance)) or (oq._oqgeneral_initialized == nil) then
    return ;
  end
...
function oq.create_raid_listing( parent, x, y, cx, cy, token, type ) 
  ...
  f.zones     = f.zones     or oq.label  ( f, x2, 2, 165, cy, ""  ) ;  x2 = x2 + 150 ;
  ...
  if (f.req_but == nil) then
    f.req_but   = oq.button ( f, x2, 2,  65, cy-2, OQ.BUT_WAITLIST, 
                                                function(self) 
                                                  oq.get_battle_tag() ;
  ...
  if (f.unlist_but == nil) then
    f.unlist_but = oq.texture_button( f, x2, 2,  24, 24, OQ.X_BUTTON_UP, OQ.X_BUTTON_DN, OQ.X_BUTTON_UP, 
...
function oq.create_tab1_common( parent )
  ...
  parent._resize = function(self)
    ...
    oq.move_y( self._voip             , cy-25*2-80 ) ;
    oq.move_y( self._lang             , cy-25*2-80 ) ;    
...
function oq.create_tab3()
  ...
  oq.premade_type_selection( OQ_data._premade_type or OQ.TYPE_NONE ) ;
...
function oq.on_player_enter_world()
  ...
  oq.timer( "join_OQGeneral", 15, oq.initial_join_oqgeneral, nil  ) ; 
end
  ...
  if (oq.toon) then
    ...
    if (oq.raid.raid_token) then
      if (oq.iam_raid_leader()) then
        ...
        oq.check_for_deserter() ;
        oq.tab3._create_but:SetText( OQ.UPDATE_BUTTON ) ;
        oq.tab3._type._edit:SetText( OQ.premade_selections[ oq.raid.type ] or "" ) ;
...
function oq.mini_count_create()
  local d = oq.CreateFrame( "Frame", "OQMiniCount", oq.mini ) ;
  d:SetPoint( "TOPLEFT", oq.mini, "TOPLEFT", -10, -15 ) ;
  d:SetWidth ( 18 ) ;
  d:SetHeight( 16 ) ;
  ...
  i:SetAllPoints( d ) ;
  i:SetAlpha( 1.0 ) ;
  i:SetTexture(   0/255,   0/255, 100/255 ) ; -- yellow background
  d.texture = i ;
  d.txt = oq.label( d, 0, 0, 18, 16, "", "CENTER", "CENTER", "GameFontNormalSmall", "MEDIUM" ) ; 
  d.txt:SetAllPoints( i ) ;
