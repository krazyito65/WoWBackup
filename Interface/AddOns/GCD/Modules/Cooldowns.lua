					...
					add = {
						...
						set = function(info, value)
									if not tonumber(value) then
										local link = GetSpellLink(addon:GetSpellPosInSpellbook(GetSpellInfo(value)), BOOKTYPE_SPELL)
										local spellString = link:match("^|c%x+|H(spell[%d:]+)|h%[")
										value = spellString:sub(7)
									end
...
function module:ACTIONBAR_UPDATE_COOLDOWN()
	if spellNum then
		local _, gcdLeft
		local spell = GetSpellBookItemName(spellNum, BOOKTYPE_SPELL) 
		gcdLeft =  GetSpellCooldown(spell)
		for _,v in ipairs(cdFrames) do
