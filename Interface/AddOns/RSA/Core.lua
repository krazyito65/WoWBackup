	...
	profile = {
		...
		Hunter = {
			...
			Spells = {
				...
				EternalGuardian = {
					Messages = {
						Start = "Resurrecting [TARGET] with [LINK]!",
					},
					CustomChannel = {
						Enabled = false,
						Channel = "",
...
--[[				DivineProtection = {
		...
		Shaman = {
			...
			Spells = {
				...
				Reincarnation = {
					Messages = {
						Start = "Resurrecting myself with [LINK]!",
					},
					CustomChannel = {
						Enabled = false,
						Channel = "",
...
function RSA:OnInitialize() -- Do all this when the addon loads.
	...
	if tonumber(self.db.global.version) < 3.103 then -- Print when addon is updated from an old version.
...
function RSA.AnnouncementCheck() -- Checks against user settings to see if we are allowed to announce.
	...
	if RSA.db.profile.General.GlobalAnnouncements.InLFG and ((InstanceType == "party" and LFParty) or (InstanceType == "raid" and LFRaid)) then return true end
	if RSA.db.profile.General.GlobalAnnouncements.InWorld and InstanceType == "none" and not LFRaid then return true end
	return false
end
...
local function General_Spells(self, _, timestamp, event, hideCaster, sourceGUID, source, sourceFlags, sourceRaidFlag, destGUID, dest, destFlags, destRaidFlags, spellID, spellName, spellSchool, missType, ex2, ex3, ex4)
	...
	if UnitPlayerOrPetInParty(source) or UnitInRaid(source) or RSA.AffiliationMine(sourceFlags) then
		if event == "SPELL_CAST_START" then
			if RSA.CanAnnounce() and (spellID == 57426 or spellID == 57426 or spellID == 87643 or spellID == 87915 or spellID == 87644
					...
					or spellID == 126497 or spellID == 126503 or spellID == 126501 or spellID == 126492 or spellID == 160914
					or spellID == 160740 or spellID == 175215) then -- FEASTS
			...
			if spellID == 83968 and RSA.IsMe(sourceFlags) then -- MASS RESURRECTION
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
		...
		if event == "SPELL_SUMMON" then
			if (spellID == 67826 or spellID == 44389 or spellID == 54711 or spellID == 22700 or spellID == 157066) and RSA.CanAnnounce() then -- JEEVES
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
			...
			if (spellID == 126459 or spellID == 161414) and RSA.IsMe(sourceFlags) then -- BLINGTRON
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
				blingtronguids[destGUID] = {["source"] = source, ["spellID"] = spellID }
				if RSA.db.profile.General.Spells.Blingtron.Messages.Start ~= "" then
					if RSA.db.profile.General.Spells.Blingtron.Local == true then
						RSA.Print_LibSink(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Blingtron.Yell == true then
						RSA.Print_Yell(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Blingtron.CustomChannel.Enabled == true then
						RSA.Print_Channel(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace), RSA.db.profile.General.Spells.Blingtron.CustomChannel.Channel)
					end
					if RSA.db.profile.General.Spells.Blingtron.Say == true then
						RSA.Print_Say(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true then
						RSA.Print_SmartGroup(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Blingtron.Party == true then
						if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true and GetNumGroupMembers() == 0 and InstanceType ~= "arena" then return end
						RSA.Print_Party(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Blingtron.Raid == true then
						if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true and GetNumGroupMembers() > 0 then return end
						RSA.Print_Raid(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.Start, ".%a+.", RSA.String_Replace))
					end
				end
			end -- BLINGTRON
		end -- IF EVENT IS SPELL_SUMMON
		if event == "SPELL_CREATE" then
			if (spellID == 54710 or spellID == 156756)and RSA.IsMe(sourceFlags) then -- MOLL-E
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
				molleguids[destGUID] = {["source"] = source, ["spellID"] = spellID }
				if RSA.db.profile.General.Spells.Molle.Messages.Start ~= "" then
					if RSA.db.profile.General.Spells.Molle.Local == true then
						RSA.Print_LibSink(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Molle.Yell == true then
						RSA.Print_Yell(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Molle.CustomChannel.Enabled == true then
						RSA.Print_Channel(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace), RSA.db.profile.General.Spells.Molle.CustomChannel.Channel)
					end
					if RSA.db.profile.General.Spells.Molle.Say == true then
						RSA.Print_Say(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Molle.SmartGroup == true then
						RSA.Print_SmartGroup(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Molle.Party == true then
						if RSA.db.profile.General.Spells.Molle.SmartGroup == true and GetNumGroupMembers() == 0 and InstanceType ~= "arena" then return end
						RSA.Print_Party(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
					if RSA.db.profile.General.Spells.Molle.Raid == true then
						if RSA.db.profile.General.Spells.Molle.SmartGroup == true and GetNumGroupMembers() > 0 then return end
						RSA.Print_Raid(string.gsub(RSA.db.profile.General.Spells.Molle.Messages.Start, ".%a+.", RSA.String_Replace))
					end
				end
			end -- MOLLE
		...
		if event == "SPELL_CAST_SUCCESS" then
			...
			if (spellID == 28880 or spellID == 59547 or spellID == 59544 or spellID == 59542 or spellID == 59548 or spellID == 59543 or spellID == 59545) and RSA.IsMe(sourceFlags) then -- GIFT OF THE NAARU
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
			...
			if spellID == 59752 and RSA.IsMe(sourceFlags) then -- EVERY MAN FOR HIMSELF
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo,}
			...
			if spellID == 20549 and RSA.IsMe(sourceFlags) then -- WARSTOMP
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo,}
			...
			if spellID == 7744 and RSA.IsMe(sourceFlags) then -- WILL OF THE FORSAKEN
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
			...
			if spellID == 20589 and RSA.IsMe(sourceFlags) then -- ESCAPE ARTIST
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
			...
			if (spellID == 78993 or spellID == 105701 or spellID == 156432) and RSA.IsMe(sourceFlags) then -- POTION OF FOCUS / CONCENTRATION
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
			...
			if (spellID == 28730 or spellID == 50613 or spellID == 80483 or spellID == 25046 or spellID == 69179) and RSA.IsMe(sourceFlags) then -- ARCANE TORRENT
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
		...
		if event == "SPELL_AURA_APPLIED" then
			if spellID == 65116 and RSA.IsMe(sourceFlags) then -- STONEFORM
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo,}
		...
		if event == "SPELL_AURA_REMOVED" then
			if spellID == 59547 and RSA.IsMe(sourceFlags) then -- GIFT OF THE NAARU
				spellinfo = GetSpellInfo(spellID)
				spelllinkinfo = GetSpellLink(spellID)
				RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = source,}
	...
	if (spellID == 134847 or spellID == 166302) and blingtronguids[destGUID] then -- BLINGTRON
			spellinfo = GetSpellInfo(blingtronguids[destGUID]["spellID"])
			spelllinkinfo = GetSpellLink(blingtronguids[destGUID]["spellID"])
			RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = blingtronguids[destGUID]["source"],}
			blingtronguids[destGUID] = nil
			if RSA.db.profile.General.Spells.Blingtron.Messages.End ~= "" then
				if RSA.db.profile.General.Spells.Blingtron.Local == true then
					RSA.Print_LibSink(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
				if RSA.db.profile.General.Spells.Blingtron.Yell == true then
					RSA.Print_Yell(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
				if RSA.db.profile.General.Spells.Blingtron.CustomChannel.Enabled == true then
					RSA.Print_Channel(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace), RSA.db.profile.General.Spells.Blingtron.CustomChannel.Channel)
				end
				if RSA.db.profile.General.Spells.Blingtron.Say == true then
					RSA.Print_Say(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
				if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true then
					RSA.Print_SmartGroup(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
				if RSA.db.profile.General.Spells.Blingtron.Party == true then
					if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true and GetNumGroupMembers() == 0 and InstanceType ~= "arena" then return end
					RSA.Print_Party(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
				if RSA.db.profile.General.Spells.Blingtron.Raid == true then
					if RSA.db.profile.General.Spells.Blingtron.SmartGroup == true and GetNumGroupMembers() > 0 then return end
					RSA.Print_Raid(string.gsub(RSA.db.profile.General.Spells.Blingtron.Messages.End, ".%a+.", RSA.String_Replace))
				end
			end
		end -- BLINGTRON
	...
	if event == "UNIT_DIED" then
		...
		if blingtronguids[destGUID] then -- BLINGTRON
			spellinfo = GetSpellInfo(blingtronguids[destGUID]["spellID"])
			spelllinkinfo = GetSpellLink(blingtronguids[destGUID]["spellID"])
			RSA.Replacements = {["[SPELL]"] = spellinfo, ["[LINK]"] = spelllinkinfo, ["[TARGET]"] = blingtronguids[destGUID]["source"],}
			blingtronguids[destGUID] = nil
