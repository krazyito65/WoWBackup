...
if rscflaskcheckb[8]==0 then
	...
	rscflasktfoodadd:SetText(format(rscpartanonsfoodadd,"+100"))
	...
	rscflasktfoodadd2:SetText(format(rscpartanonsfoodadd,"+75"))
...
if #rscignorezone10[1]>0 then
	for ug=1,#rscignorezone10[1] do
		if rscignorezone10[1][ug]==curZoneID then
			rscignorezone10[2][ug]=GetMapNameByID(GetCurrentMapAreaID())
			mojn=0
		end
	end
end
...
if nn==1 then
	local rrr=0
	if #rscignorezone10[1]>0 then
		for i=1,#rscignorezone10[1] do
			if rscignorezone10[1][i]==curZoneID then
				rscignorezone10[2][i]=GetMapNameByID(GetCurrentMapAreaID())
				rrr=1
			end
		end
	end
	if rrr==0 then
		table.insert(rscignorezone10[2],GetMapNameByID(GetCurrentMapAreaID()))
		table.insert(rscignorezone10[1],curZoneID)
	...
	if #rscignorezone10[1]>0 then
		for i=1,#rscignorezone10[1] do
			if rscignorezone10[1][i]==curZoneID then
				...
				table.remove(rscignorezone10[1],i)
				table.remove(rscignorezone10[2],i)
				i=1000
			end
		end
	end
...
if nn==3 then
	local txt=""
	if #rscignorezone10[2]>0 then
		for i=1,#rscignorezone10[2] do
			txt=txt.."|cff00ff00"..rscignorezone10[2][i].."|r"
			if #rscignorezone10[2]>i then
				txt=txt..", "
			end
		end
	end
...
table.wipe(rscignorezone10[1])
table.wipe(rscignorezone10[2])
else
--тыкни еще раз!
